...
static esp_err_t root_handler(httpd_req_t *req) {
    ...
    snprintf(status_html, sizeof(status_html),
        ...
        (approtect == 0xFFFFFFFF) ? "" :
            ((approtect == 0x0000005A || approtect == 0xFFFFFF5A) ? "" : "warning"),
        (approtect == 0xFFFFFFFF) ? "Disabled (Open for debug)" :
            ((approtect == 0x0000005A || approtect == 0xFFFFFF5A) ? "HwDisabled (Hardware unlocked)" :
                "ENABLED (Protection active!)"),
        esp_get_free_heap_size());
    
    httpd_resp_send_chunk(req, status_html, strlen(status_html));

    // Add this new status display section after the System Status table:
    const char* swd_status_section =
        "<h2>SWD Debug Status</h2>"
        "<div class='section' id='swdStatusSection'>"
        "<pre id='swdRegisterDump' style='font-family:monospace;font-size:12px;background:#f5f5f5;padding:10px;'>"
        "SWD status will appear here after connection check..."
        "</pre>"
        "</div>";

    httpd_resp_send_chunk(req, swd_status_section, strlen(swd_status_section));

    // Protection management section
    const char* protection_html = 
        "<h2>Flash Operations</h2>"
        "<div class='section'>"
        "<p class='warning'>Warning: Mass erase will DELETE ALL DATA on the chip!</p>"
    ...
    const char* script =
        ...
        ""
        "function checkSWD(){"
        "console.log('Checking SWD status...');"
        ...
        ".then(r=>r.json())"
        ".then(data=>{"
        "console.log('SWD data received:',data);"
        "let html='=== SWD Status ===\\n\\n';"
        "if(data.connected){"
        "html+='Connection: CONNECTED\\n';"
        "html+='APPROTECT: '+data.approtect+' - '+data.approtect_status+'\\n';"
        "html+='NVMC Ready: '+(data.nvmc_ready?'YES':'NO')+'\\n';"
        "html+='NVMC State: '+data.nvmc_state+'\\n';"
        "html+='Core Halted: '+(data.core_halted?'YES':'NO')+'\\n';"
        "if(data.device_id)html+='Device ID: '+data.device_id+'\\n';"
        "if(data.flash_size)html+='Flash Size: '+(data.flash_size/1024)+' KB\\n';"
        "if(data.ram_size)html+='RAM Size: '+(data.ram_size/1024)+' KB\\n';"
        "html+='\\n=== Registers ===\\n';"
        "if(data.registers){"
        "for(let key in data.registers){"
        "html+=key.padEnd(20)+': '+data.registers[key]+'\\n';"
        "}"
        "}else{"
        "html+='No register data available\\n';"
        "}"
        ...
        "}"
        "}else{"
        "html+='Connection: DISCONNECTED\\n';"
        "if(data.error)html+='Error: '+data.error+'\\n';"
        ...
        "})"
        ".catch(err=>{"
        "console.error('SWD check error:',err);"
        "document.getElementById('protStatus').innerText='Error checking SWD';"
        "document.getElementById('swdRegisterDump').textContent='Error fetching SWD status: '+err;"
        "});"
        "}"
        ""
        "function massErase(){"
        ...
        "}"
        ""
        "// Check SWD status when page loads"
        "window.addEventListener('DOMContentLoaded',function(){"
        "console.log('Page loaded, checking SWD...');"
        "setTimeout(checkSWD,500);"
        "});"
        "</script>"
        "</body></html>";
